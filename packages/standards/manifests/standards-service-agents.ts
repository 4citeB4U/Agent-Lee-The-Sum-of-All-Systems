...
REGION: CORE
TAG: CORE.MODULE.STANDARDS_SERVICE_AGENTS.MAIN
...
WHERE = packages/standards/manifests/standards-service-agents.ts
WHEN = 2026-04-18
...
type StandardsServiceAgent = {
  agentId: string;
  title: string;
  purpose: string;
  authority: "info" | "warn" | "blocking";
  watchScope: string[];
  mode: Array<"scan" | "purge-plan" | "enforce" | "quarantine" | "move-simulate" | "move-analyze" | "move-group-analyze" | "move-resolve-blockers" | "move-apply" | "watch" | "ci">;
  packagePath: string;
  entrypoint: string;
  receiptAgent: string;
...
const buildRequiredVmSurface = (agentId: string, title: string) => ({
  surfaceId: `vm.${agentId}`,
  title: `${title} VM`,
  route: `/construct/agent-vm/${agentId}`,
  required: true as const,
  fullIdentitySurface: true as const,
  standardsMonitored: true as const,
  monitorScope: [
    "authority",
    "mission",
    "receipts",
...
export function assertStandardsAgentsHaveVmIdentity(agents: StandardsServiceAgent[]): StandardsServiceAgent[] {
...
export const STANDARDS_SERVICE_AGENTS: StandardsServiceAgent[] = assertStandardsAgentsHaveVmIdentity([
  {
