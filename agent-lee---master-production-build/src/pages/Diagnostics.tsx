...
import { PerspectiveCamera, OrbitControls, Stars, Environment, Sparkles, Html, Text, Line as DreiLine } from '@react-three/drei';
...
import { 
  ...
  BookMarked, Gauge, Braces, ChevronRight, Archive as ArchiveIcon
} from 'lucide-react';
...
// --- Components ---
function VoxelPart({ w, h, d, position, rotation = [0, 0, 0], material, edgeMaterial, label, ...props }: any) {
  const geometry = useMemo(() => new THREE.BoxGeometry(w, h, d, w, h, d), [w, h, d]);
  const edges = useMemo(() => new THREE.EdgesGeometry(geometry), [geometry]);
  return (
    <mesh position={position} rotation={rotation} geometry={geometry} material={material} {...props}>
...
function Brain({ onCenterClick, onMetallicClick, isMobile = false }: { onCenterClick?: () => void; onMetallicClick?: () => void; isMobile?: boolean }) {
  ...
  const stream3Positions = useMemo(() => new Float32Array(STREAM_COUNT * 3), []);
  const stream3Ref = useRef<THREE.Points>(null);
  ...
  const basePlatMat = useMemo(() => new THREE.MeshStandardMaterial({
    color: 0x2a2a2a, metalness: 0.9, roughness: 0.2, emissive: 0x0088ff, emissiveIntensity: 0.05
  }), []);
  ...
  useFrame((state) => {
    ...
    if (stream1Ref.current && stream2Ref.current && stream3Ref.current && characterGroupRef.current && baseGroupRef.current) {
      ...
      [stream1Ref, stream2Ref, stream3Ref].forEach((ref, si) => {
        const offset = si * 0.33;
        const posAttr = ref.current!.geometry.attributes.position;
        const mat = ref.current!.material as THREE.PointsMaterial;
        
        mat.opacity = (si === 0 ? 0.9 : 0.6) * pulse;
        mat.size = (si === 0 ? 0.8 : 0.4) * pulse;

        for (let k = 0; k < STREAM_COUNT; k++) {
          const frac = ((t * 1.2 + offset + k / STREAM_COUNT) % 1);
          _streamP.lerpVectors(_streamStart, _streamEnd, frac);
          
          // Add some jitter for "data" look
          const jitter = 0.15;
          posAttr.setXYZ(
            k, 
            _streamP.x + (Math.random() - 0.5) * jitter, 
            _streamP.y, 
            _streamP.z + (Math.random() - 0.5) * jitter
          );
        }
    ...
    <group scale={0.2} position={[0, -2, 0]}>
      <group ref={characterGroupRef}>
        ...
        <group 
          ...
          position={[0, -14, 0]}
        >
          {/* Fail-safe Hit Box for Metallic Cubes (Diagnostics) */}
          <mesh 
            onPointerDown={(e) => {
              e.stopPropagation();
              console.log('Metallic base pointer down, opening Diagnostics');
              onMetallicClick?.();
            }}
            onClick={(e) => {
              e.stopPropagation();
              console.log('Metallic base clicked, opening Diagnostics');
              onMetallicClick?.();
            }}
              ...
              Array.from({ length: 4 }).map((_, z) => {
                ...
                return (
                  <VoxelPart 
                    key={`${x}-${y}-${z}`}
                    w={1.2} h={1.2} d={1.2} 
                    position={[(x - 1.5) * 2.2, (y - 1.5) * 2.2, (z - 1.5) * 2.2]} 
                    material={basePlatMat} 
          ...
          {/* Center Core Trigger (Pallium) - Placed last to ensure priority in some raycasting scenarios */}
          <VoxelPart 
            w={5} h={5} d={5} 
            position={[0, 0, 0]} 
            ...
            onPointerDown={(e: any) => {
              e.stopPropagation();
              console.log('Center blue core pointer down, opening Pallium');
              onCenterClick?.();
            }}
            onClick={(e: any) => {
              e.stopPropagation();
              console.log('Center blue core clicked, opening Pallium');
              onCenterClick?.();
            }}
            onPointerOver={() => (document.body.style.cursor = 'pointer')}
            onPointerOut={() => (document.body.style.cursor = 'auto')}
      ...
      <points ref={stream1Ref}>
        ...
        <pointsMaterial size={0.8} color={0x00f2ff} transparent opacity={0.9} blending={THREE.AdditiveBlending} depthWrite={false} />
      </points>
      <points ref={stream2Ref}>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" args={[stream2Positions, 3]} />
        </bufferGeometry>
        <pointsMaterial size={0.4} color={0xa855f7} transparent opacity={0.6} blending={THREE.AdditiveBlending} depthWrite={false} />
      </points>
      <points ref={stream3Ref}>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" args={[stream3Positions, 3]} />
        </bufferGeometry>
        <pointsMaterial size={0.3} color={0xffffff} transparent opacity={0.5} blending={THREE.AdditiveBlending} depthWrite={false} />
      </points>
    </group>
  );
}
...
function AgentNode({ agent, onClick, isSelected, isMerged, hideLabel }: any) {
  ...
  return (
    <group ref={groupRef} position={position}>
      ...
      {!isMerged && !hideLabel && (
        <Html distanceFactor={14} center visible={!hideLabel}>
          <div 
...
function AgentLeeDiagnostics({ agent: initialAgent, onClose }: AgentLeeDiagnosticsProps) {
  ...
  return (
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[800] bg-[#020408] backdrop-blur-xl overflow-y-auto"
        onClick={onClose}
      >
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:h-[90vh] md:max-h-[1000px] md:overflow-hidden">
            <div className="w-full md:w-[400px] md:shrink-0 border-b md:border-b-0 md:border-r border-white/5 bg-black/80 p-8 md:p-10 flex flex-col items-center gap-8 md:gap-10 md:overflow-y-auto scrollbar-hide">
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-8 md:p-12 scrollbar-hide">
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-8">
              ...
              <MetricCard icon={Gauge} label="Cognitive Latency" value={`12ms`} color="#F59E0B" sub="Local-First Response" />
            </div>
...
export default function DiagnosticsPage() {
  ...
  const [isMemoryLakeOpen, setIsMemoryLakeOpen] = useState(false);
  ...
  return (
    <div className="w-full bg-[#020408] overflow-hidden relative" style={{ height: '100dvh' }}>
      <div className="absolute top-0 left-0 w-full px-8 py-6 z-20 flex justify-between items-center">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate('/')} 
            className="pointer-events-auto px-4 py-1.5 rounded-full text-[9px] font-mono uppercase tracking-widest transition-all active:scale-95 border border-[#00f2ff]/30 bg-[#00f2ff]/05 text-[#00f2ff] hover:bg-[#00f2ff]/15"
          >
            ← Motherboard
          </button>
          <p className="text-[#00f2ff] text-xs tracking-widest font-black uppercase">Diagnostic Center</p>
        </div>
        <div className="flex items-center gap-3">
          <button 
            onClick={() => setIsMemoryLakeOpen(true)} 
            className="pointer-events-auto px-6 py-2 rounded-xl border border-cyan-500/30 bg-cyan-500/5 text-cyan-400 text-[10px] font-black uppercase tracking-widest hover:bg-cyan-500/20 hover:border-cyan-500/60 transition-all shadow-[0_0_20px_rgba(6,182,212,0.1)]"
          >
            Pallium
          </button>
          <button 
            onClick={() => setIsSystemReportOpen(true)} 
            className="pointer-events-auto px-6 py-2 rounded-xl border border-[#00f2ff]/30 bg-white/5 text-[#00f2ff] text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all"
          >
            Diagnostics
          </button>
          <button 
            ...
            className="pointer-events-auto px-6 py-2 rounded-xl border border-[#00f2ff]/30 bg-white/5 text-[#00f2ff] text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all"
          >
            {isMerged ? 'Expand' : 'Merge'}
          </button>
        </div>
      </div>
      <Canvas shadows dpr={[1, 2]} gl={{ powerPreference: 'high-performance' }}>
        <PerspectiveCamera makeDefault position={[0, 3, 22]} fov={55} />
        <OrbitControls enablePan={false} autoRotate={!selectedAgent && !isMerged} />
        <ambientLight intensity={0.2} />
        <Stars radius={100} depth={50} count={5000} factor={4} />
        <group visible={!selectedAgent && !isSystemReportOpen}>
          <Brain 
            onCenterClick={() => setIsMemoryLakeOpen(true)} 
            onMetallicClick={() => setIsSystemReportOpen(true)}
            isMobile={isMobile} 
          />
          {AGENTS.map((agent) => (
            <AgentNode 
              ...
              isMerged={isMerged} 
              hideLabel={!!selectedAgent || isSystemReportOpen}
            />
          ))}
        </group>
        <EffectComposer><Bloom luminanceThreshold={0.85} intensity={0.5} /></EffectComposer>
      </Canvas>
      <AnimatePresence>
        {isMemoryLakeOpen && (
          <MemoryLake onClose={() => setIsMemoryLakeOpen(false)} />
        )}
      </AnimatePresence>

      <AnimatePresence>
        {selectedAgent && (
          <AgentLeeDiagnostics agent={selectedAgent} onClose={() => setSelectedAgent(null)} />
        )}
      </AnimatePresence>
      <SystemDiagnostics isOpen={isSystemReportOpen} onClose={() => setIsSystemReportOpen(false)} />
    </div>
  );
}
